...
class DataSetViewer:
    ...
    def create_ui(self):
        ...
        scrollball = ttk.Scrollbar(main_panel, orient = VERTICAL, command = self.tree.yview)
        self.tree.configure(yscroll = scrollball)
        #scrollball.grid(row=0, column = 1, sticky = "ns")
        scrollball.pack(side = RIGHT, fill = BOTH, expand = True)
    def show_ui(self):
        self.root.mainloop()
